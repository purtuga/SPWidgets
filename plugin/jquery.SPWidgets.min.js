(function(g){(function(){try{if(!g.pt)g.pt={}}catch(b){g.pt={}}if(g.pt._cache===undefined)g.pt._cache={};g.SPWidgets={};g.SPWidgets.version="20130629045258";g.SPWidgets.defaults={};g.fn.SPMsgHasError=function(){var c=g(this).find("ErrorCode"),f=false;if(!c.length)return g(this).find("faultcode").length?true:false;c.each(function(){if(g(this).text()!=="0x00000000"){f=true;return false}});return f};g.fn.SPGetMsgError=function(){var c=g(this),f="",h=c.find("ErrorCode"),d=0;h.length||(h=c.find("faultcode"));
if(!h.length)return"";h.each(function(){var e=g(this);if(e.text()!=="0x00000000"){d+=1;f+="("+d+") "+e.text()+": "+e.parent().children().not(e).text()+"\n"}});return f=d+" error(s) encountered! \n"+f};g.SPWidgets.fillTemplate=function(c,f){var h={},d,e,r,a,t;h.response="";h.template=String(g("<div/>").append(c).html());h.tokens=h.template.match(/(\{\{.*?\}\})/g);g.isArray(f)||(f=[f]);if(h.tokens!==null){r=0;for(a=f.length;r<a;r++){t=h.template;d=0;for(e=h.tokens.length;d<e;d++){h.tokens[d]=h.tokens[d].replace(/[\{\{\}\}]/g,
"");t=t.replace("{{"+h.tokens[d]+"}}",f[r][h.tokens[d]])}h.response+=t}}return h.response};g.SPWidgets.parseLookupFieldValue=function(c){var f=[],h=String(c).split(";#"),d=h.length,e,r;if(c===undefined)return f;for(c=0;c<d;c++){e=h[c];c++;r=h[c];if(e||r)f.push({id:e,title:r})}return f};g.SPWidgets.getCamlLogical=function(c){c=g.extend({},{type:"AND",values:[],onEachValue:null},c);var f="<And>",h="</And>",d="",e=0,r=0,a=false;c.type=String(c.type).toUpperCase();if(!g.isArray(c.values))c.values=[c.values];
if(c.type!=="AND"){f="<Or>";h="</Or>"}d=f;e=c.values.length;r=e-1;a=g.isFunction(c.onEachValue);if(e<2)d="";for(f=0;f<e;f++){d+=a?String(c.onEachValue(c.values[f])).toString():String(c.values[f]).toString();if(r-f>1){d+=g.SPWidgets.getCamlLogical(g.extend({},c,{values:c.values.splice(f+1,e-f)}));break}}if(e>1)d+=h;return d};g.SPWidgets.SPGetDateString=function(c,f){function h(e){return e<10?"0"+e:e}f=String(f||"local").toLowerCase();c=c||new Date;var d="";return d=f==="utc"?c.getUTCFullYear()+"-"+
h(c.getUTCMonth()+1)+"-"+h(c.getUTCDate())+"T"+h(c.getUTCHours())+":"+h(c.getUTCMinutes())+":"+h(c.getUTCSeconds())+"Z":c.getFullYear()+"-"+h(c.getMonth()+1)+"-"+h(c.getDate())+"T"+h(c.getHours())+":"+h(c.getMinutes())+":"+h(c.getSeconds())};g.SPWidgets.makeSameHeight=function(c,f){var h=0,d=g(c);d.each(function(){var e=g(this).css("height","");if(h<e.outerHeight(true))h=e.outerHeight(true)});if(h>0){if(f)h+=f;d.height(h)}return c};g.SPWidgets.escapeXML=function(c){if(typeof c!=="string")return"";
return c.replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/'/g,"&apos;").replace(/"/g,"&quot;")};g.SPWidgets.unEscapeXML=function(c){if(typeof c!=="string")return"";return c.replace(/&lt;/g,"<").replace(/&gt;/g,">").replace(/&amp;/g,"&").replace(/&apos;/g,"'").replace(/&quot;/g,'"')}})(g);(function(b){var c={};c.initDone=false;c.maxColumns=20;b.SPWidgets.defaults.board={list:"",field:"",CAMLQuery:"<Query></Query>",CAMLViewFields:"",fieldFilter:null,optionalLabel:"(none)",template:null,
webURL:b().SPServices.SPGetCurrentSite(),showColPicker:false,colPickerLabel:"Columns",colPickerVisible:[],colPickerCloseLabel:"Close",colPickerApplyLabel:"Apply",colPickerCheckLabel:"Check-Uncheck All",colPickerTotalLabel:"Selected.",colPickerMaxColMsg:"Can not exceed 10 columns!",colPickerMinColMsg:"Mininum of 2 required!",onGetListItems:null,onPreUpdate:null,onBoardCreate:null};b.fn.SPShowBoard=function(f){if(!c.initDone){c.initDone=true;c.styleSheet!==""&&b('<style type="text/css">\n\n'+c.styleSheet+
"\n\n</style>").prependTo("head")}var h=arguments;return this.each(function(){var d=b(this),e=typeof f==="string",r=d.hasClass("hasSPShowBoard"),a=null,t="",n=null;if(r&&!e)return this;else if(e&&r&&!d.hasClass("loadingSPShowBoard")){t=f.toLowerCase();n=d.data("SPShowBoardOptions");if(t==="refresh")n._getListItems().then(function(){n.showItemsOnBoard({refresh:true})});else if(t==="redraw")n.setBoardColumnHeight();else t==="setvisible"&&n.showColPicker&&n.setUserDefinedVisibleCol(h[1]);return this}if(d.hasClass("loadingSPShowBoard"))return this;
a=b.extend({},b.SPWidgets.defaults.board,f,{ele:d,states:[],statesMap:{},tmpltHeader:"",tmpltState:"",statesCntr:"",headersCntr:"",listItems:[],initDone:false,formUrls:null,isStateRequired:true,maxColumnVisible:10,showNumberOfColumns:10,getBoardStates:function(){return b.Deferred(function(i){b().SPServices({operation:"GetList",listName:a.list,cacheXML:true,async:false,webURL:a.webURL,completefunc:function(j,m){var k=b(j.responseXML),l=k.find("Fields Field[StaticName='"+a.field+"']");if(!l.length){l=
...
b(this);s=v.attr("ows_ID");v=v.attr("ows_"+l.attr("ShowField"));s=s+";#"+v;a.states.push({type:"lookup",title:v,name:s});a.statesMap[s]=a.states[a.states.length-1]});i.resolveWith(a,[p,q])}}}})}}})}).promise()},_getListItems:function(){return b.Deferred(function(i){function j(m){b.isFunction(a.onGetListItems)&&a.onGetListItems.call(d,a.listItems,m);i.resolveWith(d,[a.listItems])}b.isFunction(a.CAMLQuery)?a.CAMLQuery.call(d,function(m){if(b.isArray(m)){a.listItems=m;j(a.CAMLQuery)}},f):b().SPServices({operation:"GetListItems",
...
b.fn.SPLookupField=function(f){if(!c._islookupFieldCssDone){c._islookupFieldCssDone=true;b('<style type="text/css">\n\n'+c.styleSheet+"\n\n</style>").prependTo("head")}var h=arguments;this.each(function(){var d=b(this);if(!d.is("input")&&!d.is("textarea")||d.hasClass("hasLookupSPField")){if(typeof f==="string"&&d.is("input")){var e=d.data("SPWidgetLookupFieldUI").data("SPWidgetLookupFieldOpt");if(f.toLowerCase()==="method"){d=String(h[1]||"").toLowerCase();var r=h[2]||{};if(d==="clear")if(r.id===
undefined){e._cntr.find("div.spwidgets-lookup-selected").css("display","none").empty();e._ele.val("");e._cntr.find("div.spwidgets-lookup-input").css("display","")}else alert("TBD... Delete individual ID's")}}return this}e=b.extend({},b.SPWidgets.defaults.LookupField,f,{_ele:d.css("display","none").addClass("hasLookupSPField")});e.showSelectedItems=function(n,i){var j=e._selectedItemsCntr.css("display",""),m=[],k=false;if(!j.find("div.spwidgets-item").length||e.allowMultiples===false)j.empty();if(b.isArray(n))m=
...
g.pt.pickSPUser={_isPickSPUserCssDone:false};g.fn.pickSPUser=function(b){if(!g.pt.pickSPUser._isPickSPUserCssDone){g.pt.pickSPUser._isPickSPUserCssDone=true;g('<style type="text/css">\n\n'+g.pt.pickSPUser.styleSheet+"\n\n</style>").prependTo("head")}var c=arguments;this.each(function(){var f=g(this);if(!f.is("input")||f.hasClass("hasPickSPUser"))return typeof b==="string"&&f.is("input")?g.pt.pickSPUser.handleAction.apply(this,c):this;var h=g.extend({},{allowMultiples:true,maxSearchResults:50,webURL:g().SPServices.SPGetCurrentSite(),
onPickUser:null,onCreate:null,onRemoveUser:null,inputPlaceholder:"Type and Pick"},b,{eleUserInput:f.css("display","none").addClass("hasPickSPUser")});h.maxSearchResults=parseInt(h.maxSearchResults)||50;var d=g(g.pt.pickSPUser.htmlTemplate).find(".pt-pickSPUser").clone(1).insertAfter(f);h.eleSelected=d.find("div.pt-pickSPUser-selected").empty();h.elePickInput=d.find("div.pt-pickSPUser-input");h.addPeopleToList=function(r,a){var t=(new String(r)).split(";#"),n=t.length,i,j,m;for(i=0;i<n;i++){j=t[i];
i++;m=t[i];j=g.pt.pickSPUser.getUserHtmlElement(h,j,m).appendTo(h.eleSelected);(function(k,l){h.getSearchResults(l).done(function(p){var q=String(l).toLowerCase();g.each(p,function(u,s){if(String(s.displayName).toLowerCase()===q){k.data("pickspuser_object",s);return false}})})})(j,m)}g.pt.addHoverEffect(h.eleSelected.find("div.pt-pickSPUser-person-cntr"));h.allowMultiples===false&&h.elePickInput.css("display","none");g.pt.pickSPUser.storeListOfUsers(h.eleSelected,a)};h.getSearchResults=function(r){return g.Deferred(function(a){g().SPServices({operation:"SearchPrincipals",
searchText:r,maxResults:h.maxSearchResults,async:true,webURL:h.webURL,completefunc:function(t,n){var i=[];g(t.responseXML).find("PrincipalInfo").each(function(){var j=g(this);i.push({displayName:j.find("DisplayName").text(),accountId:j.find("UserInfoID").text(),accountName:j.find("AccountName").text(),accountType:j.find("PrincipalType").text(),value:j.find("DisplayName").text(),label:j.find("DisplayName").text()})});a.resolveWith(t,[i,t,n])}})}).promise()};h.allowMultiples===true&&h.eleSelected.addClass("pt-pickSPUser-selected-multiple");
f.val()&&h.addPeopleToList(f.val(),noEvents);var e={};h.elePickInput.find("input[name='pickSPUserInputField']").attr("placeholder",h.inputPlaceholder).autocomplete({minLength:3,source:function(r,a){if(r.term in e)a(e[r.term]);else{e[r.term]=[];h.getSearchResults(r.term).then(function(t){e[r.term].push.apply(e[r.term],t);a(e[r.term])})}},select:function(r,a){if(h.allowMultiples===false)h.eleSelected.empty();else if(h.eleSelected.find("div[data-pickspuserid='"+a.item.accountId+"']").length)return;g.pt.pickSPUser.getUserHtmlElement(h,
a.item.accountId,a.item.displayName).appendTo(h.eleSelected).data("pickspuser_object",a.item);g.pt.pickSPUser.storeListOfUsers(d);g.pt.addHoverEffect(d.find("div.pt-pickSPUser-person-cntr"));setTimeout(function(){r.target.value=""},50);h.allowMultiples===false&&h.elePickInput.hide();g.isFunction(h.onPickUser)&&h.onPickUser.call(h.eleUserInput,g.extend({},a.item));f.trigger(g.Event("spwidget:peoplePickerAdd"),[h.eleUserInput,g.extend({},a.item)])}});d.data("pickSPUserContainerOpt",h);f.data("pickSPUserContainer",
d);g.isFunction(h.onCreate)&&h.onCreate.call(f,f);f.trigger(g.Event("spwidget:peoplePickerCreate"),[h.eleUserInput]);return this});return this};g.pt.pickSPUser.getUserHtmlElement=function(b,c,f){b=g(g.pt.pickSPUser.htmlTemplate).find(".pt-pickSPUser-person").clone(1);b.attr("data-pickSPUserID",c);b.find("span.pt-person-name").append(f).end().attr("data-pickSPUserNAME",f);return b};g.pt.pickSPUser.removeUser=function(b){var c=g(b).closest("div.pt-pickSPUser"),f=c.data("pickSPUserContainerOpt");b=g(b).closest("div.pt-pickSPUser-person");
var h=b.data("pickspuser_object");g.isFunction(f.onRemoveUser)&&f.onRemoveUser.call(f.ele,f.ele,b,h);b.fadeOut("fast",function(){g(this).remove();g.pt.pickSPUser.storeListOfUsers(c)});f.allowMultiples===false&&f.elePickInput.show("fast",function(){f.elePickInput.find("input").focus()});f.eleUserInput.trigger(g.Event("spwidget:peoplePickerRemove"),[f.eleUserInput,h])};g.pt.pickSPUser.storeListOfUsers=function(b,c){var f=g(b).closest("div.pt-pickSPUser"),h=f.data("pickSPUserContainerOpt"),d="",e={};
f.find("div.pt-pickSPUser-selected div.pt-pickSPUser-person").each(function(){if(!e[g(this).attr("data-pickSPUserID")]){e[g(this).attr("data-pickSPUserID")]=true;if(d)d+=";#";d+=g(this).attr("data-pickSPUserID");d+=";#";d+=g(this).attr("data-pickSPUserNAME")}});h.eleUserInput.val(d);c||h.eleUserInput.change()};g.pt.pickSPUser.handleAction=function(b,c,f){b=String(b).toLowerCase();c=String(c).toLowerCase();o=g(this).data("pickSPUserContainer").data("pickSPUserContainerOpt");ret=this;if(b==="method")switch(c){case "clear":o.eleUserInput.val("");
o.eleSelected.empty();if(o.allowMultiples===false){o.eleSelected.css("display","none");o.elePickInput.show()}break;case "destroy":g(this).hasClass("hasPickSPUser")&&g(this).removeClass("hasPickSPUser").next(".pt-pickSPUser").remove().show().trigger("change");break;case "add":o.addPeopleToList(f);break;case "remove":if(f){b=o.eleSelected.find("div[data-pickspuserid='"+f+"']");b.length||(b=o.eleSelected.find("div[data-pickspusername='"+f+"']"));b.length&&g.pt.pickSPUser.removeUser(b)}break;case "getSelected":ret=
g.SPWidgets.parseLookupFieldValue(o.eleUserInput.val())}return ret};g.pt.pickSPUser.styleSheet="/**\n * Styles for the Pick User Widget\n */\n.pt-pickSPUser .pt-pickSPUser-selected-multiple {\n    min-height: 3em;\n}\n\n.pt-pickSPUser .pt-pickSPUser-selected .pt-pickSPUser-person {\n    float: left;\n    margin-left: .2em;\n}\n.pt-pickSPUser .pt-pickSPUser-hint {\n    font-size: .9em;\n}\n\n\n.pt-pickSPUser .pt-pickSPUser-person-cntr {\n    margin: .2em 0em;\n    padding: .2em;\n    position: relative;\n}\n\n.pt-pickSPUser .pt-pickSPUser-person-cntr .pt-person-name {\n    padding-right: 2em;\n}\n\n/* Item action container (delete button) */\n.pt-pickSPUser .pt-pickSPUser-person-cntr .pt-pickSPUser-person-actions {\n    position: absolute;\n    right: 1px;\n    top: 1px;\n    padding: .2em;\n    display: none;\n}\n.pt-pickSPUser .pt-pickSPUser-person-cntr .pt-pickSPUser-person-actions .pt-pickSPUser-person-action-links,\n.pt-pickSPUser .pt-pickSPUser-person-cntr .pt-pickSPUser-person-actions .pt-pickSPUser-person-action-links .tt-confirm-delete {\n    float:right;\n}\n\n/* Make the action visible if we hover or we are trying to confirm a deletion */\n.pt-pickSPUser .pt-pickSPUser-person-cntr.ui-state-hover .pt-pickSPUser-person-actions,\n.pt-pickSPUser .pt-pickSPUser-person-cntr .pt-pickSPUser-person-actions.tt-confirm,\n.pt-pickSPUser .pt-pickSPUser-person-cntr .pt-pickSPUser-person-actions a {\n    display:block;\n    float: right;\n}\n\n/* autocomplete busy image */\n.ui-autocomplete-loading {\n    background: white url('/_layouts/images/loading.gif') right center no-repeat;\n}\n\n\n";
g.pt.pickSPUser.htmlTemplate='<!--\n    Html Templates for the PickSPUser plugin.\n    \n    |\n    |   $Author$\n    | $Revision$\n    |     $Date$\n    |       $Id$\n    |\n--\>\n<div>\n    <div class="pt-pickSPUser">\n        <div class="pt-pickSPUser-selected">\n            None Selected!\n        </div>\n        <div style="clear:both"></div>\n        <div class="pt-pickSPUser-input" \n                title="Type user name above to view search results.">\n            <input name="pickSPUserInputField" value="" type="text"/>\n        </div>\n    </div>\n    \n    <div class="pt-pickSPUser-person">\n        <div class="pt-pickSPUser-person-cntr ui-state-default ui-corner-all">\n            <span class="pt-person-name"></span>\n            <div class="pt-pickSPUser-person-actions">\n                <div class="tt-record-item-action-links">\n                    <a class="tt-delete-icon" href="javascript:" onclick="jQuery.pt.pickSPUser.removeUser(this);">\n                        <img style="border: medium none; margin-right: 2px;" alt="Delete" src="/_layouts/images/delitem.gif">\n                    </a>\n                    <div style="clear:both;"></div>\n                </div>\n                <div style="clear:both;"></div>\n            </div>\n        </div>\n    </div>\n</div>\n';
g.pt.addHoverEffect=function(b){return g(b).each(function(){if(!g(this).hasClass("addHoverEffectDone")){g(this).addClass("addHoverEffectDone");var c=this;g(c).mouseenter(function(){g(c).toggleClass("ui-state-hover")});g(c).mouseleave(function(){g(c).toggleClass("ui-state-hover")})}})};g.pt._isSPUploadCssDone=false;g.fn.SPControlUpload=function(b){var c=g.extend({},{listName:"",folderPath:"",uploadDonePage:"/_layouts/viewlsts.aspx",onPageChange:null,uploadUrlOpt:"",overwrite:false,uploadPage:"/_layouts/Upload.aspx",
overlayClass:"",overlayBgColor:"white",overlayMessage:'<div class="loadingOverlayMsg">Loading...</div>'},b);if(!g.pt._isSPUploadCssDone){g.pt._isSPUploadCssDone=true;g('<style type="text/css">\n\n'+g.pt.SPUploadStyleSheet+"\n\n</style>").prependTo("head")}if(c.listName&&c.listName.indexOf("{")!=0)c.listName=g.pt.getListUID(c.listName);if(!c.listName){g(this).html('<div class="ui-state-error">Input parameter [listName] not valid!</div>');return this}c.siteUrl=g().SPServices.SPGetCurrentSite();if(String(c.uploadPage).toLowerCase().indexOf("http")==
-1){b="/";if(c.uploadPage.indexOf("/")==0)b="";c.uploadPage=c.siteUrl+b+c.uploadPage;c.uploadPage2=c.siteUrl+"/_layouts/UploadEx.aspx"}if(String(c.uploadDonePage).toLowerCase().indexOf("http")==-1){b="/";if(c.uploadDonePage.indexOf("/")==0)b="";c.uploadDonePage=c.siteUrl+b+c.uploadDonePage}c._uploadUrlParams="?List="+g.pt.getEscapedUrl(c.listName)+"&RootFolder="+g.pt.getEscapedUrl(c.folderPath)+"&Source="+g.pt.getEscapedUrl(c.uploadDonePage)+"&"+c.uploadUrlOpt;c.uploadPage+=c._uploadUrlParams;c.uploadPage2+=
c._uploadUrlParams;c._lastError="";c._reloadCount=0;c.ev={state:1,action:"uploading",hideOverlay:true,pageUrl:"",page:null,isUploadDone:false};var f="";if(c.overlayBgColor)f="style='background-color:"+c.overlayBgColor+";' ";g(this).each(function(){var h=g(this),d=h.outerHeight()-75,e={top:0,left:0,height:h.outerHeight(true)-15,display:"block"};h.empty().addClass("hasSPControlUploadUI").append("<div class='SPControlUploadUI spcontrolupload'><div class='mainContainer'><div class='iFrameWindow'><iframe name='SPControlUploadUI' frameborder='0' scrollbars='yes' scrolling='yes'></iframe></div><div class='buttonPane'><button type='button' class='ui-state-default' name='upload_button' value='upload' onclick='$.pt._onUpload(this);'>Upload</button></div><div class='loadingOverlay "+
c.overlayClass+"' "+f+"></div></div></div>").find(".SPControlUploadUI").data("SPControlUploadOptions",c).end().find("iframe").css("height",d).load(function(){g.pt._onIFramePageChange(h.find(".SPControlUploadUI"))}).attr("src",c.uploadPage).end().find(".loadingOverlay").append(c.overlayMessage).css(e);return this});return this};g.pt._onUpload=function(b){var c=g(b).closest(".SPControlUploadUI"),f=c.find("iframe").contents(),h=f.find("input[type='file']").closest("tr").siblings().find("span");b=c.data("SPControlUploadOptions");
var d=b.ev;h.css("display","none");if(f.find("input[type='file']").val()){d.state=2;d.action="preLoad";if(b.onPageChange)if(b.onPageChange.call(c.closest(".hasSPControlUploadUI"),d)===false)return false;c.find(".buttonPane").css("display","none").end().find(".loadingOverlay").fadeIn("slow",function(){f.find("input[type='button'][id$='btnOK']").click();if(h.is(":visible")){c.find(".loadingOverlay").css("display","none").end().find(".buttonPane").show(0).end();return false}})}};g.pt._onIFramePageChange=
function(b){var c=g(b).closest(".SPControlUploadUI"),f=c.find("iframe").contents(),h=c.data("SPControlUploadOptions"),d=h.ev;d.pageUrl=f[0].location.href;d.page=f;setTimeout(function(){if(g.pt.isSameUrlpage(g.pt.getUnEscapedUrl(d.pageUrl),g.pt.getUnEscapedUrl(h.uploadPage))||g.pt.isSameUrlpage(g.pt.getUnEscapedUrl(d.pageUrl),g.pt.getUnEscapedUrl(h.uploadPage2))){f.find("form").children(":visible").hide();f.find("form").append("<div id='SPControlUploadModUI' style='position:absolute;width:99.9%;height:99.9%x;left:0px;top:0px;background-color:white;padding-top:3em;'></div>");
if(RegExp(/error/i).test(g.trim(f.find(".ms-pagetitle").text()))||RegExp(/error/i).test(g.trim(f.find("title").text()))||RegExp(/error\.aspx/i).test(g.trim(f.find("form").attr("action")))){h._lastError=f.find("[id$='LabelMessage']").text();if(h._reloadCount>1){alert("Error encountered during upload which is causing program to loop. Last upload error was: "+h._lastError);c.find(".loadingOverlay").fadeOut();return}h._reloadCount+=1;c.find("iframe").attr("src",h.uploadPage);return}else{if(f.find("#GearPage")&&
!f.find("input[type='file']").length)return;f.find("input[type='file']").closest("table").appendTo(f.find("#SPControlUploadModUI")).removeClass("ms-authoringcontrols").find("a[id$='UploadMultipleLink']").closest("tr").css("display","none").end().end().find("input[type='checkbox'][name$='OverwriteSingle']").closest("tr").css("display","none").end().end().find(".ms-fileinput").css("font-size","13pt");if(h._lastError){f.find("input[type='file']").after('<div style="color:red;"><div class="ui-state-error">ERROR: '+
h._lastError+"</div></div>");h._lastError=""}h._reloadCount=0;h.overwrite?f.find("input[type='checkbox'][name$='OverwriteSingle']").prop("checked","checked"):f.find("input[type='checkbox'][name$='OverwriteSingle']").prop("checked","");c.find(".buttonPane").show();d.state=1;d.action="postLoad";d.hideOverlay=true}}else{d.state=3;d.action="postLoad";d.hideOverlay=true;var e=f.find("form").eq(0),r=e.prop("onsubmit");if(g.pt.isSameUrlpage(d.pageUrl,h.uploadDonePage)){d.isUploadDone=true;d.hideOverlay=
false;c.find(".loadingOverlay").empty().append('<div class="ui-state-highlight" style="width:80%;">Upload Complete!</div>')}e[0].onsubmit=function(){c.find(".loadingOverlay").css("display","block");var a=true;if(g.isFunction(h.onPageChange))a=h.onPageChange.call(c.closest(".hasSPControlUploadUI"),g.extend({},d,{state:3,action:"preLoad"}));if(a===false){c.find(".loadingOverlay").fadeOut();return a}if(g.isFunction(r))a=r();if(a===false){c.find(".loadingOverlay").fadeOut();return a}return a};g(c.find("iframe")[0].contentWindow).unload(function(){c.find(".loadingOverlay").css("display",
"block");if(g.isFunction(h.onPageChange))return h.onPageChange.call(c.closest(".hasSPControlUploadUI"),g.extend({},d,{state:3,action:"preLoad"}))})}h.onPageChange&&h.onPageChange.call(c.closest(".hasSPControlUploadUI"),d);if(d.action.toLowerCase()!="postload"||d.hideOverlay==true)c.find(".loadingOverlay").fadeOut()},500)};g.pt.isSameUrlpage=function(b,c){if(!b||!c)return false;var f=b;if(b.indexOf("?")>-1)f=b.substring(0,b.indexOf("?"));return c.indexOf(f)==0?true:false};g.pt.getEscapedUrl=escapeProperly;
g.pt.getUnEscapedUrl=unescapeProperly;g.pt.getListUID=function(b){if(!b)return"";var c="";if(g.pt._cache["getListUID:"+b])return c=g.pt._cache["getListUID:"+b];g().SPServices({operation:"GetList",listName:b,async:false,completefunc:function(f){c=g(f.responseXML).find("List").attr("ID")}});if(c)g.pt._cache["getListUID:"+b]=c;return c};g.pt.SPUploadStyleSheet="/**\n * FILE: jquery.SPControlUpload.css\n * \n * \n */\n.spcontrolupload .mainContainer {\n\tposition: relative;\n\tdisplay:block;\n}\n.spcontrolupload iframe {\n\toverflow: auto;\n\twidth: 99%;\n\theight: 99%;\n}\n.spcontrolupload iframe .ms-fileinput {\n\tfont-size:14pt;\n}\n\n.spcontrolupload .buttonPane {\n\tdisplay: none;\n}\n.spcontrolupload button {\n\tpadding: .5em;\n\tmargin: .2em;\n}\n.spcontrolupload .loadingOverlay {\n\tposition: absolute;\n\twidth: 100%;\n\theight: 100%;\n\tdisplay: none;\n}\n.spcontrolupload .loadingOverlayMsg {\n\tmargin: auto;\n\tpadding: 5em;\n\tfont-size: 1.2em;\n}\n\n";
(function(b){var c={};c.isInitDone=false;c.templates=null;b.SPWidgets.defaults.filter={list:"",webURL:b().SPServices.SPGetCurrentSite(),columns:["Title"],textFieldTooltip:"Use a semicolon to delimiter multiple keywords.",showFilterButton:true,showFilterButtonTop:true,showStackedUI:false,filterButtonLabel:"Filter",onFilterClick:null,onReady:null,onReset:null,ignoreKeywords:/^(of|and|a|an|to|by|the|or|from)$/i};b.fn.SPFilterPanel=function(f){if(!c.isInitDone){c.isInitDone=true;c.styleSheet!==""&&b('<style type="text/css">\n\n'+
c.styleSheet+"\n\n</style>").prependTo("head");c.templates=b(c.htmlTemplate)}if(typeof f==="string"){if(!this.eq(0).hasClass("hasSPFilterPanel"))return;return function(h){h=h.eq(0).find("div.spwidget-filter").data("SPFilterPanelInst");var d=f.toLowerCase(),e=h.$ele;switch(d){case "getfilter":e=c.getFilterValues(h);break;case "reset":c.doResetFilter(h);break;case "destroy":h.$ele.removeClass("hasSPFilterPanel").empty()}return e}(this)}return this.each(function(){var h=b.extend({},b.SPWidgets.defaults.filter,
f),d={$ele:b(this),$ui:null,opt:h};d.getListDefinition=function(){return b.Deferred(function(e){b().SPServices({operation:"GetList",listName:h.list,cacheXML:true,async:true,webURL:h.webURL,completefunc:function(r,a){var t=b(r.responseXML);if(a==="error")e.rejectWith(t,[r,a]);else t.SPMsgHasError()?e.rejectWith(t,[r,a]):e.resolveWith(t,[r,a])}})}).promise()};d.buildWidget=function(){return b.Deferred(function(e){d.getListDefinition().then(function(){var r=this,a="",t=c.templates.filter("#filter_column").html();
d.$ui=b(c.templates.filter("#filter_main_ui").html()).appendTo(d.$ele.empty().addClass("hasSPFilterPanel"));d.opt.showStackedUI&&d.$ui.find("div.spwidget-filter-column-cntr").addClass("spwidget-filter-fmt-stacked");d.$list=r;b.each(d.opt.columns,function(n,i){var j=r.find("Field[DisplayName='"+i+"']"),m=t,k="",l={type:null};j.length||(j=r.find("Field[Name='"+i+"']"));if(j.length){l.Name=j.attr("Name");switch(j.attr("Type")){case "Choice":j.find("CHOICES CHOICE").each(function(p,q){k+=b.SPWidgets.fillTemplate(c.templates.filter("#filter_choice_field").html(),
...
this.SPGetMsgError()+"</div>");e.reject()})}).promise()};if(d.opt.ignoreKeywords&&!d.opt.ignoreKeywords instanceof RegExp)d.opt.ignoreKeywords=/Inst.opt.ignoreKeywords/i;d.buildWidget();return this})};c.onFilterTypeChange=function(){var f=b(this),h=f.closest("div.spwidget-column").find("div.spwidget-filter-value-cntr"),d=h.find(".spwidget-input");f=f.val();if(f==="IsNull"||f==="IsNotNull"){h.addClass("spwidget-disabled");d.attr("disabled","disabled")}else{h.removeClass("spwidget-disabled");d.removeAttr("disabled",
"disabled")}return this};c.onFilterButtonClick=function(){var f=b(this).closest("div.spwidget-filter").data("SPFilterPanelInst"),h=null;if(b.isFunction(f.opt.onFilterClick)){h=c.getFilterValues(f);f.opt.onFilterClick.call(f.$ele,h)}return this};c.doResetFilter=function(f){if(b.isFunction(f.onReset))if(f.onReset.call(f.$ele,c.getFilterValues(f))===true)return f;f.$ui.find("div[data-spwidget_column_type='text'] input").val("").end().find("div[data-spwidget_column_type='choice'] input").prop("checked",
false).end().find(".hasPickSPUser").pickSPUser("method","clear").end().find(".hasLookupSPField").SPLookupField("method","clear");return f};c.getFilterValues=function(f){function h(r){return b.SPWidgets.getCamlLogical({type:"OR",values:r.values,onEachValue:function(a){return"<"+r.matchType+"><FieldRef Name='"+r.columnName+"' /><Value Type='Text'>"+b.SPWidgets.escapeXML(a)+"</Value></"+r.matchType+">"}})}var d={CAMLQuery:"",filters:{},count:0},e=[];f.$ui.find("div.spwidget-column").each(function(r,
a){var t=b(a),n=t.find(".spwidget-input"),i=n.attr("name"),j={columnName:i,matchType:t.find("select.spwidget-filter-type").val(),values:[],count:0,CAMLQuery:""};t=t.data("spwidget_column_type");if(j.matchType==="IsNull"||j.matchType==="IsNotNull"){j.CAMLQuery="<"+j.matchType+"><FieldRef Name='"+i+"' /></"+j.matchType+">";j.count+=1}else switch(t){case "choice":n.each(function(){var m=b(this),k=m.val();m.is(":checked")&&j.values.push(k)});if(j.values.length){j.count=j.values.length;j.CAMLQuery=h(j)}break;
